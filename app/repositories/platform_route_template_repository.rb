...
class PlatformRouteTemplateRepository < ApplicationRepository
  ...
  def import(records)
    PlatformRouteTemplate.import records, on_duplicate_key_update: { conflict_target: [:guid, :project_id], columns: [:platform_company_outlet_id, :description, :route_no, :is_deleted] }, returning: :guid
  end
end
